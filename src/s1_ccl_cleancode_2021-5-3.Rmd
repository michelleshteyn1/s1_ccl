...
The hypothesis was not supported: participants in the similarity condition neither showed significantly greater projection (M = 0.0937, SD = .45) than those in the dissimilarity condition (M = 0.0788, SD = .43), t(158) = .21, p = .99, nor significantly less stereotyping (M = 0.4990, SD = 0.47) than those in the dissimilarity condition (M = 0.5638, SD = 0.43), t(158) = -.92, p = .80. This result was confirmed by the two-way mixed ANOVA: no significant interaction was found between experimental condition and the within-subjects factor on projection and stereotyping scores, F(1,158) = .68, p = .41.
...
# Save the ggplot
ggsave(here::here("figures", "ames_anova_viz_all_within.png"), width = 7, height = 5)
...
# Save the ggplot
ggsave(here::here("figures", "ames_anova_viz_between_withinerrorbars.png"), width = 7, height = 5)
...
libcon_subset <- key_variables %>% 
  select(id, lib_con, exp_condition, projection, stereotyping) %>% 
  unique()
...
## Condition*Ideology on Projection
...
## Condition*Ideology on Stereotyping
...
##Error bars represent standard error of the mean
p1<-ggplot(lib_con_projection, aes(x=lib_con, y=projection, fill=exp_condition)) + 
    geom_bar(position=position_dodge(), stat="identity",
             colour="black", # Use black outlines,
             size=.3) + # Thinner lines
...
# Save the ggplot
ggsave(here::here("figures", "libcon_projection.png"), width = 7, height = 5)
...
##Error bars represent standard error of the mean
p2<-ggplot(lib_con_stereotyping, aes(x=lib_con, y=stereotyping, fill=exp_condition)) + 
    geom_bar(position=position_dodge(), stat="identity",
             colour="black", # Use black outlines,
             size=.3) + # Thinner lines
...
# Save the ggplot
ggsave(here::here("figures", "libcon_stereotyping.png"), width = 7, height = 5)

```
*Note*. Error bars show standard errors. 101 Liberals and 40 Moderates/Conservatives were included in the analysis.


## Make combined figure of interactions using `patchwork()`
```{r, include=FALSE}
library(patchwork)

 (p1 / p2 ) 

ggsave(here::here("figures","combined_libcon.png"), width = 7, height = 10)

 (p1 | p2 ) 

ggsave(here::here("figures","combined_libcon_horizontal.png"), width = 12, height = 5)
```
