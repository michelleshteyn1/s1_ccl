...
For each participant, a multiple regression predicting target responses with self-responses and typical business owner responses was computed (across the 8 reasons to support the Energy Innovation & Carbon Dividend Act). The mean standardized beta weights were then used as measures of projection (the self-response beta) and stereotyping (the typical businessman response beta)
...
# Use `pivot_longer()` to make the data tidy and have variables for: reason_num, self_rank, stereotype_rank, robert_rank
...
stereotyping_projection_subset<-ccl_long %>% 
  select(id,reason_number,s,g,r)


#This is the closest example to my data I think
anscombe %>%
