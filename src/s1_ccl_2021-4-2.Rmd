...
Group the data by `exp_condition` and `ranking`, and then compute some summary statistics of the `score` variable: mean and sd. 
...
anova_data2 %>% 
  ...
  shapiro_test(score)
```
This shows that the scores for projection and stereotyping were NOT normally distributed at each ranking point (*p* < .05)
...
The between groups test indicates that the variable `exp_condition` is not significant, consequently in the graph we see that the lines for the two groups are overlapping. The within subject test indicate that there is significant ranking effect, in other words, the groups change in score over time. In the graph we see that the groups have lines that have a gradual positive slope. Also, since the lines are parallel, we are not surprised that the interaction between time and group is not significant.
