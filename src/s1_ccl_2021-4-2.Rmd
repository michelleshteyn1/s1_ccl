...
### Create binary variables for gender and ideology

`gender_bin` (Male/Female) and `lib_con` (Very liberal, liberal = Liberal, Moderate, Conservative, & Very conservative = Conservative)
...
ccl <- ccl %>% 
  mutate(gender_bin = case_when(
    ...
    gender=="Male"~"Male"),
  gender_bin = as.factor(gender_bin),
  lib_con = case_when(
...
Averaging the top 3 reasons for self and the top 3 reasons for the stereotypical businessman can serve as secondary DVs of projection and stereotyping. First, I will calculate the Cronbach's alpha for 6,7,8 and 1,2,3 to show that the two groups of reasons hang together (for 6,7,8 across self, group, and Robert rankings, alpha = .48; for 1,2,3 across self, group, and Robert rankings, alpha = .46). Then, I'll average the two groups and treat them both like scales.
...
robert_123<-ccl %>% 
  select(r1,r2,r3)
#alpha(robert_123) #alpha = .15, 95% CI [-.08, .37]
...
#alpha(sgr_678) #alpha = .48, 95% CI [.36, .60]

sgr_123 <- ccl %>% 
  select(s1,s2,s3,g1,g2,g3,r1,r2,r3)
#alpha(sgr_123) #alpha = .46, 95% CI [.34, .59]
...
stereotyping_projection_subset<-ccl_long %>% 
  select(id,reason_number,group,s,g,r, gender_bin, lib_con, projection_678, stereotyping_123) %>% 
...
for (i in 1:nrow(stereotyping_projection_subset)) {
  ...
  stereotyping_projection_subset[i, 11]<-temp_self_rankings
  stereotyping_projection_subset[i, 12]<-temp_stereotype_rankings
}
...
anova_data <- stereotyping_projection_subset %>% 
  select(id, group, projection, stereotyping, projection_678, stereotyping_123) %>% 
...
#Create subset with just the variables I'm interested in
zero_order<-ccl_long %>% 
  select(id,reason_number,group,s,g,r, lib_con, gender_bin, projection_678, stereotyping_123) %>% 
...
# For loop will iterate through each row of the dataframe
# The scope of every action within the for loop is limited to that single iteration, so no need to manually filter data and calculate regression for each participant
for (i in 1:nrow(zero_order)) {
  
  # This creates a dataframe of one row, the row that the for loop is currently on
  id<-zero_order %>% 
    filter(id==zero_order[[i, 1]])
  ...
  # The last two lines store the variables created above in columns in the original dataframe in the row that the for loop is currently on
  zero_order[i, 11]<-temp_self_robert
  zero_order[i, 12]<-temp_stereotype_robert
  zero_order[i, 13]<-temp_self_stereotype
}

# Keep variables needed for analysis and get rid of duplicate rows
zero_order <- zero_order %>% 
  select(id,exp_condition,lib_con, gender_bin, projection_678, stereotyping_123, self_robert, stereotype_robert, self_stereotype) %>% 
  unique()
...
```{r}
hist(zero_order$self_robert)
hist(zero_order$self_stereotype)
hist(zero_order$stereotype_robert)
```
...
```{r, warning = FALSE, message=FALSE}
mean_freq_self <- stereotyping_projection_subset %>% 
  group_by(reason_number) %>% 
  summarize(mean_self_ranking = mean(self_rankings))

mean_freq_stereotype<- stereotyping_projection_subset %>% 
  group_by(reason_number) %>% 
  summarize(mean_stereotype_ranking = mean(stereotype_rankings))

mean_freq_robert<- stereotyping_projection_subset %>% 
  group_by(reason_number) %>% 
...
Put original MLR beta analyses in primary results section, put correlational analyses as footnote. Do independent samples t-tests for all 3 zero-order correlations to see if they significantly differ between exp condition:

## Independent samples t-tests on zero-order

- Effect of experimental condition on correlation between self-robert: N.S., *p* = 0.53  
- Effect of experimental condition on correlation between self-stereotype: N.S., *p* = 0.17 (this doesn't even make sense because people received the experiment after they rated both self and stereotype)  
...
res <- t.test(self_robert ~ exp_condition, data = zero_order, var.equal = TRUE)
#res # N.S., p = 0.53

# Compute independent samples t-test of self_stereotype predicted by exp_condition 
res1 <- t.test(self_stereotype~ exp_condition, data = zero_order, var.equal = TRUE)
#res1 # N.S., p = 0.17

# Compute independent samples t-test of stereotype_robert predicted by exp_condition 
res2 <- t.test(stereotype_robert~ exp_condition, data = zero_order, var.equal = TRUE)
#res2 # N.S., p = 0.21
...
- `self_robert`: *t*(1,161) = -652.48, *p* < .0001, d = -51.26 (large)  
- `self_stereotype`: *t*(1,161) = -614.39, *p* < .0001, d = -48.27 (large)  
- `stereotype_robert`: *t*(1,161) = -815.25, *p* < .0001, d = -64.05 (large)   
...
- Effect of gender and condition on ***projection**: N.S. interaction, *p* = .70. But INTERESTING - the effect of gender on projection approaches significance, *F*(1,157) = 2.78, *p* = .098. I followed this up with an independent samples t-test looking at the effect of gender alone on projection: approaches significance, *t*(1,159) = -1.68, *p* = .096 (Female *M* = .017, Male *M* = .14). So looks like males feel more similar to Robert than females and they project more.
- Effect of gender and condition on **stereotyping**: N.S. interaction, *p* = .70. N.S. effect of gender, *p* = .88.  

On zero-order correlations:  

- `self_robert`: N.S. interaction, *p* = .41, but the effect of gender on the relationship between self and robert is significant, *F*(1,157) = 6.491, *p* = .01. I followed this up with an independent samples t-test looking at the effect of gender alone on `self_robert`: significant, *t*(1,159) = -2.56, *p* = .01 (Female *M* = -.17, Male *M* = .03). Converges with the result on projection above.  
- `self_stereotype`: there is a significant interaction of condition and gender, *F*(1,157) = 5.354, *p* = .02.  
...
stereotyping_projection_subset <- stereotyping_projection_subset %>% 
  select(id, group, projection, stereotyping, gender_bin, lib_con) %>% 
  unique()

# Projection and Stereotyping:

#Two-way anova: Gender (M/F) and Condition (Sim/Diff) on projection:
res.aov1 <- aov(projection ~ gender_bin + group + gender_bin:group, data = stereotyping_projection_subset)
#summary(res.aov1)
...
res.aov2 <- aov(stereotyping ~ gender_bin + group + gender_bin:group, data = stereotyping_projection_subset)
#summary(res.aov2)
...
res.aov3 <- aov(self_robert ~ gender_bin + exp_condition + gender_bin:exp_condition, data = zero_order)
#summary(res.aov3)
#Do a t-test to follow up:
gender_self_robert<- t.test(self_robert ~ gender_bin, data = zero_order, var.equal = TRUE)
#gender_self_robert

#Two-way anova: Gender (M/F) and Condition (Sim/Diff) on self_stereotype:
res.aov4 <- aov(self_stereotype ~ gender_bin + exp_condition + gender_bin:exp_condition, data = zero_order)
#summary(res.aov4)
grouped_self_stereotype<-zero_order %>% 
  group_by(exp_condition, gender_bin) %>% 
  summarise(mean_self_stereotype = mean(self_stereotype, na.rm = TRUE)) %>% 
  drop_na()

ggplot(data = grouped_self_stereotype, aes(x = gender_bin, y = mean_self_stereotype, fill = exp_condition))+
  geom_bar(stat = "identity", position = "dodge")+
  labs(x = "Gender",
       y = "Mean correlation self and stereotype",
       title = "For Men, Experimental Condition affects their Stereotyping")

#Two-way anova: Gender (M/F) and Condition (Sim/Diff) on stereotype_robert:
res.aov5 <- aov(stereotype_robert ~ gender_bin + exp_condition + gender_bin:exp_condition, data = zero_order)
#summary(res.aov5)

```


## Liberal vs. conservative differences: alt DV
...
# Alternative DV: averages of 'enviro' reasons and and 'businessman' reasons

- Effect of experimental condition on use of self reasons (6,7,8, alpha = .48) for Robert: N.S., *p* = 0.15
- Effect of experimental condition on use of business-y reasons (1,2,3, alpha = .46) for Robert: N.S., *p* = 0.26
...
What's the alpha of 6,7,8? Can we make a scale of them.
What's the alpha of 1,2,3? Average them to make a scale of them. 

That's very similar to the Ames type of analysis.

Do some simple ANOVAs: is the average of 6,7,8 different for Robert in the similarity vs. difference conditions?
...
res3 <- t.test(projection_678~ exp_condition, data = zero_order, var.equal = TRUE)
#res3 # N.S., p = 0.15

# Effect of exp condition on stereotyping_123
res4 <- t.test(stereotyping_123~ exp_condition, data = zero_order, var.equal = TRUE)
#res4 # N.S., p = 0.26
```
...
# Exploratory: Normative vs. Non-normative

Code participants as concordant or non-concordant with the predominant social norm (in this population, they like camping, adventurous vacation, don't like scary movies). If they had 0 attributes that are most common, they are non-concordant; if 3, they are concordant.  
