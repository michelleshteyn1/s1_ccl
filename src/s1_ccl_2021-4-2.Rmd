...
Show that people's responses matched to Robert's on Qualtrics if in the similarity condition, and were opposite to Robert's responses if in the different condition. 
```{r}
...
# Create subset with just the variables I need for my analysis

exploratory <- exploratory %>% 
  select(id, exp_condition, self_robert, self_stereotype, stereotype_robert) %>% 
  unique()
...
In the table above, the zero-order correlation between self-robert appeared to differ in the similarity and difference conditions (although they were both close to zero). Run a t-test to see if they are significantly different (N.S., *p* = 0.53).
```{r, message=FALSE, warning=FALSE}
# Compute t-test of self_robert predicted by exp_condition 
res <- t.test(self_robert ~ exp_condition, data = exploratory, var.equal = TRUE)
res
```
...
DS notes: There are 2 ways to look at stereotyping and projection: the MLR beta way, which looks at the *relative* importance of each, or the correlation way, which looks at how the independent impact of each rating differs between conditions. 

Put original MLR beta analyses in primary results section, put correlational analyses as footnote. Do ind samples t-tests for all 3 zero-order correlations to see if they significantly differ between exp condition, and do one-sample t-tests to see if they significantly differ from zero. one sample t-test "evidence that the stereotyping is significantly different from zero, and by how much?"
