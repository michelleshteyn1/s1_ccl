...
Create `lib_con_num`, a continuous version of `ideology` from 1 (Very Liberal) to 5 (Very Conservative).
...
ccl <- ccl %>% 
  ...
  lib_con_num = as.factor(lib_con_num)) 
```
...
stereotyping_projection_subset<-ccl_long %>% 
  select(id,reason_number,group,s,g,r, gender_bin, lib_con,lib_con_num, normative, projection_678, stereotyping_123) %>% 
...
for (i in 1:nrow(stereotyping_projection_subset)) {
  ...
  stereotyping_projection_subset[i, 13]<-temp_self_rankings
  stereotyping_projection_subset[i, 14]<-temp_stereotype_rankings
}
...
There are 9 extreme outliers identified by the `identify_outliers` function: id 4, 34, 118, 49, 87, 105, 49, 64, 105. **Not going to remove them** because it was not determined a priori and the function `identify_outliers()` removes based on IQR, which is really just trimming.
```{r, warning=FALSE, message=FALSE}
anova_data2 %>% 
...
This shows that the scores for projection and stereotyping were NOT normally distributed at line 4 (*p* < .05)
```{r, warning=FALSE, message=FALSE}
anova_data2 %>% 
  group_by(exp_condition, ranking) %>% 
  shapiro_test(score)
...
I ended up using [this tutorial from UCLA.](https://stats.idre.ucla.edu/r/seminars/repeated-measures-analysis-with-r/)

The between groups test indicates that the variable `exp_condition` is not significant, *F*(1,158) = 0.27, *p* = 0.61, and consequently in the graph we see that the lines for the two groups are overlapping. The within-subjects test indicates that there is a significant effect of the within-subjects factor (reasons to support CFD); in other words, the groups change in score over time. In the graph we see that the groups have lines that have a gradual positive slope. The interaction is not significant, *F*(1,158) = 0.52, *p* = 0.47.
```{r, message=FALSE, warning=FALSE}

# Omit NAs from my data so that both lines show up on the graph
...
# ANALYSES FOR MANUSCRIPT 4/16/21

## 2 x 2 Repeated-Measures ANOVA: Ames DV

**UPDATE**. I used [this tutorial from biostats](https://biostats.w.uib.no/factorial-repeated-measures-anova-two-way-repeated-measures-anova/) to do the repeated measures ANOVA and [this tutorial from biostats](https://biostats.w.uib.no/post-hoc-tests-multiple-comparisons-in-linear-mixed-effect-models-2/) to do the multiple comparisons.

**NOTE**. I got slightly different results from this ANOVA than the one above, probably because I did it as a linear model rather than with the ANOVA function.

Should report as F(df ranking, df error) = F-value, p = p-value
...
ames_anova <- lme(score ~ ranking * exp_condition, random=~1|id, data=anova_data2)
anova(ames_anova) 

#Post-Hoc Tests: Multiple Comparisons in Linear Mixed Effect Models
#https://biostats.w.uib.no/post-hoc-tests-multiple-comparisons-in-linear-mixed-effect-models-2/
...
## T-test: Experimental Condition on Alt. DV: 3 and 3
...
```{r, message=FALSE, warning=FALSE}
alt_DV_subset <- stereotyping_projection_subset %>% 
  select(id, group, projection_678, stereotyping_123) %>% 
  unique()

# Effect of exp condition on projection_678
ttest_projection678 <- t.test(projection_678~ group, data = alt_DV_subset, var.equal = TRUE)
ttest_projection678

# Effect of exp condition on stereotyping_123
ttest_stereotyping123 <- t.test(stereotyping_123 ~ group, data = alt_DV_subset, var.equal = TRUE)
ttest_stereotyping123
```
...
```{r, message=FALSE,warning=FALSE, results='hide'}
libcon_subset <- stereotyping_projection_subset %>% 
  select(id, lib_con, lib_con_num, group, projection, stereotyping, projection_678, stereotyping_123) %>% 
  unique()

# Ames Projection and Stereotyping:

#Two-way anova: lib_con (lib vs. con) and Condition (Sim/Diff) on projection:
libcon1 <- aov(projection ~ lib_con + group + lib_con:group, data = libcon_subset)
summary(libcon1)

#Two-way anova: lib_con (lib vs. con)  and Condition (Sim/Diff) on stereotyping:
libcon2 <- aov(stereotyping ~ lib_con  + group + lib_con:group, data = libcon_subset)
summary(libcon2)

# CONTRASTS
contrasts1 <- emmeans(libcon1, pairwise ~ group*lib_con)
contrasts1

contrasts1_pairs <- emmeans(libcon1, ~ group*lib_con)
pairs(contrasts1_pairs, simple = "each")

contrasts2 <- emmeans(libcon2, pairwise ~ group*lib_con)
contrasts2

contrasts2_pairs <- emmeans(libcon2, ~ group*lib_con)
pairs(contrasts2_pairs, simple = "each")

# Linear Regression

#Two-way anova: lib_con (lib vs. con) and Condition (Sim/Diff) on projection:
libcon1a <- aov(projection ~ lib_con_num + group + lib_con_num:group, data = libcon_subset)
summary(libcon1a)

#Two-way anova: lib_con (lib vs. con)  and Condition (Sim/Diff) on stereotyping:
libcon2a <- aov(stereotyping ~ lib_con_num  + group + lib_con_num:group, data = libcon_subset)
summary(libcon2a)
...
libcon3 <- aov(projection_678 ~ lib_con + group + lib_con:group, data = libcon_subset)
summary(libcon3)

#Two-way anova: lib_con (lib vs. con)  and Condition (Sim/Diff) on stereotyping_123:
libcon4 <- aov(stereotyping_123 ~ lib_con  + group + lib_con:group, data = libcon_subset)
summary(libcon4)


# CONTRASTS
contrasts3 <- emmeans(libcon3, pairwise ~ group*lib_con)
contrasts3

contrasts3_pairs <- emmeans(libcon3, ~ group*lib_con)
pairs(contrasts3_pairs, simple = "each")

contrasts4 <- emmeans(libcon4, pairwise ~ group*lib_con)
contrasts4

contrasts4_pairs <- emmeans(libcon4, ~ group*lib_con)
pairs(contrasts4_pairs, simple = "each")

# Linear Regression

#Two-way anova: lib_con (lib vs. con) and Condition (Sim/Diff) on projection_678:
libcon3a <- aov(projection_678 ~ lib_con_num + group + lib_con_num:group, data = libcon_subset)
summary(libcon3a)

#Two-way anova: lib_con (lib vs. con)  and Condition (Sim/Diff) on stereotyping_123:
libcon4a <- aov(stereotyping_123 ~ lib_con_num  + group + lib_con_num:group, data = libcon_subset)
summary(libcon4a)
...
Also save variables on the 6 Americas.
...
#six_americas <- ccl %>% 
#   select(id, imp_gw, worried_gw, harm_personally, harm_future_gen) %>%
#    rename(casenumber = id,
#           important = imp_gw,
#          worry = worried_gw,
#           personal = harm_personally,
#           future = harm_future_gen)
#   drop_na()
...
#write_xlsx(six_americas, here("data","six_americas.xlsx"))
...
for (i in 1:nrow(stereotyping_projection_subset)) {
  ...
  # The last two lines store the variables created above in columns in the original dataframe in the row that the for loop is currently on
  stereotyping_projection_subset[i, 15]<-temp_self_robert
  stereotyping_projection_subset[i, 16]<-temp_stereotype_robert
  stereotyping_projection_subset[i, 17]<-temp_self_stereotype
}
...
#Two-way anova: lib_con (lib vs. con) and Condition (Sim/Diff) on projection:
libcon1 <- aov(projection ~ lib_con + group + lib_con:group, data = stereotyping_projection_subset)
summary(libcon1)

#Two-way anova: lib_con (lib vs. con)  and Condition (Sim/Diff) on stereotyping:
libcon2 <- aov(stereotyping ~ lib_con  + group + lib_con:group, data = stereotyping_projection_subset)
#summary(libcon2)
...
libcon3 <- aov(self_robert ~ lib_con + group + lib_con:group, data = stereotyping_projection_subset)
#summary(libcon3)

#Two-way anova: lib_con and Condition (Sim/Diff) on stereotype_robert:
libcon4 <- aov(stereotype_robert ~ lib_con  + group + lib_con:egroup, data = stereotyping_projection_subset)
#summary(libcon4)

grouped_dvs_libcon <- stereotyping_projection_subset %>% 
  group_by(lib_con, group) %>% 
...
# On projection
ggplot(data=grouped_dvs_libcon, aes(x = lib_con, y = mean_projection, fill = group))+
  geom_bar(stat = "identity", position = "dodge")+
  theme(axis.title.x = element_blank())+
...
# On alt projection
ggplot(data=grouped_dvs_libcon, aes(x = lib_con, y = mean_projection_678, fill = group))+
  geom_bar(stat = "identity", position = "dodge")+
  theme(axis.title.x = element_blank())+
...
# On stereotyping
ggplot(data=grouped_dvs_libcon, aes(x = lib_con, y = mean_stereotyping, fill = group))+
  geom_bar(stat = "identity", position = "dodge")+
  theme(axis.title.x = element_blank())+
...
# On alt stereotyping
ggplot(data=grouped_dvs_libcon, aes(x = lib_con, y = mean_stereotyping_123, fill = group))+
  geom_bar(stat = "identity", position = "dodge")+
  theme(axis.title.x = element_blank())+
...
```{r, message=FALSE, warning=FALSE}
# Effect of exp condition on projection_678
aov678 <- t.test(projection_678~ group, data = stereotyping_projection_subset, var.equal = TRUE)
#aov678 # N.S., p = 0.13

# Effect of exp condition on stereotyping_123
aov123 <- t.test(stereotyping_123~ group, data = stereotyping_projection_subset, var.equal = TRUE)
#aov123 # N.S., p = 0.28
```
...
mixed_aov <- aov(score ~ group * ranking + Error(id), data = anova_data3)
#summary(mixed_aov)
...
with(anova_data3, interaction.plot(ranking, group, score,
...
alt_dv_projection <- aov(projection_678 ~ gender_bin + group + gender_bin:group, data = stereotyping_projection_subset)
#summary(alt_dv_projection)

#Two-way anova: Gender (M/F) and Condition (Sim/Diff) on stereotyping_123:
alt_dv_stereotyping <- aov(stereotyping_123 ~ gender_bin + group + gender_bin:group, data = stereotyping_projection_subset)
...
alt_dv_projection_libcon <- aov(projection_678 ~ lib_con + group + lib_con:group, data = stereotyping_projection_subset)
#summary(alt_dv_projection_libcon)

#Two-way anova: lib_con and Condition (Sim/Diff) on stereotyping_123:
alt_dv_stereotyping_libcon <- aov(stereotyping_123 ~ lib_con + group + lib_con:group, data = stereotyping_projection_subset)
...
normative <- aov(projection_678 ~ normative + group + normative:group, data = stereotyping_projection_subset)
#summary(normative)

#Two-way anova: normative and Condition (Sim/Diff) on stereotyping_123:
normative1 <- aov(stereotyping_123 ~ normative + group + normative:group, data = stereotyping_projection_subset)
#summary(normative1)
...
```{r}
ggplot(data=normative_grouped, aes(x = normative, y = mean_projection_678, fill = group))+
  geom_bar(stat = "identity", position = "dodge")+
  theme(axis.title.x = element_blank())+
...
```{r}
ggplot(data=normative_grouped, aes(x = normative, y = mean_stereotyping_123, fill = group))+
  geom_bar(stat = "identity", position = "dodge")+
  theme(axis.title.x = element_blank())+
